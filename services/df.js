// const dialogflow = require('@google-cloud/dialogflow') ; 
// // credentials
// const CREDENTIALS = {"type": "service_account",
//     "project_id": "chatbot-auds",
//     "private_key_id": "cfc8b81e2e81e12dbe22b6b8ae765c0003f95e57",
//     "private_key": "-----BEGIN PRIVATE KEY-----\\n-----END PRIVATE KEY-----\n",
//     "client_email": "chatbot-bot@chatbot-auds.iam.gserviceaccount.com",
//     "client_id": "116166319528895296705",
//     "auth_uri": "https://accounts.google.com/o/oauth2/auth",
//     "token_uri": "https://oauth2.googleapis.com/token",
//     "auth_provider_x509_cert_url": "https://www.googleapis.com/oauth2/v1/certs",
//     "client_x509_cert_url": "https://www.googleapis.com/robot/v1/metadata/x509/chatbot-bot%40chatbot-auds.iam.gserviceaccount.com"
//   }
// const projectId  = CREDENTIALS.project_id ; 
// const configuration  = {credentials : { private_key: CREDENTIALS['private_key'],  client_email:CREDENTIALS['client_email']}}
// const sessionClient = new dialogflow.SessionsClient(configuration) ; 


// async function detectIntent2(query, contexts = []) {
//     let sessionPath = sessionClient.projectAgentSessionPath(projectId, sessionId);
//     const request = {
//       session: sessionPath,
//       queryInput: {
//         text: {
//           text: query,
//           languageCode: 'en-US', // replace with the language code of your Dialogflow agent
//         },
//       },
//       queryParams: {
//         contexts: contexts, // pass in the previous contexts to check for follow-up intents
//       },
//     };
  
//     const responses = await sessionClient.detectIntent(request);
//     const result = responses[0].queryResult;
  
//     const intentName = result.intent.displayName;
//     const intentParams = result.parameters.fields;
//     const intentFollowup = result.outputContexts.find(context => {
//       return context.name.includes(`${intentName}-followup`);
//     });
  
//     if (intentFollowup) {
//       const followupParams = intentFollowup.parameters.fields;
//       return {
//         intentName: intentFollowup.name.split('/').pop().split('-followup')[0],
//         params: followupParams,
//         contexts: result.outputContexts,
//       };
//     } else {
//       return {
//         intentName: intentName,
//         params: intentParams,
//         contexts: result.outputContexts,
//       };
//     }
//   }
  
//   module.exports = detectIntent2;